   ...
   public function actionCetakWord($id)
    {
        ...
        $query = new Query;
        $query	->select([
		'kapal.kapal_nama AS kapal_nama', 
                'kapal.kapal_neg AS kapal_neg', 
                'tagen.namaagen AS namaagen']
		
		)  
	->from('tmaster')
	->join('INNER JOIN', 'kapal',
				'kapal.kapal_id =tmaster.idkapal')		
	->join('INNER JOIN', 'tagen', 
				'tagen.id =tmaster.idagen')
	->LIMIT(2)	; 
		
        $command = $query->createCommand();
        $datatabel = $command->queryAll();
          */
	...
	$template = Yii::getAlias('@hscstudio/export').'/templates/opentbs/lhpspword1.docx';
      ...
      /*
       $data = [];
        foreach($datatabel as $datatabels){
            $data[] = [
                'kapal_nama'=> $datatabels->kapal_nama,
                'kapal_neg'=>$datatabels->kapal_neg,
                'namaagen'=>$datatabels->namaagen
                    ];
        }
        */ 
